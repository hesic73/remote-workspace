...
fn setup() -> Option<Env> {
    ...
    let artifact = "remote-workspace-server-linux-x86_64-musl";
...
impl Env {
    fn add(&self, name: &str, root: &str) -> std::process::Output {
        Command::new(env!("CARGO_BIN_EXE_remote-workspace"))
            .args([
                "workspace",
                "add",
                name,
            ...
            .env("HOME", &self.home)
            .env("PATH", &self.path)
            .env("REMOTE_WORKSPACE_RELEASE_BASE", &self.release)
...
fn add_installs_probes_and_records() {
    ...
    assert!(!fleet.contains("remote_shell"));
    ...
    assert!(env
        .home
        .join(".local/lib/remote-workspace/remote-workspace-server")
        .exists());
