...
#[derive(Debug, Parser)]
#[command(
    name = "remote-workspace-mcp",
    version,
    about = "MCP server exposing named remote-workspace workspaces"
)]
struct Cli {
    /// Path to the fleet config (TOML) declaring the workspaces. Defaults to
    /// ~/.remote-workspace/workspaces.toml.
    #[arg(long)]
    ...
    if cli.check {
        ...
        for (name, ws) in &fleet {
            let location = match &ws.endpoint {
                remote_workspace_client::Endpoint::Ssh { host, root, .. } => {
                    format!("{host}:{root}")
                }
                remote_workspace_client::Endpoint::Local { root, .. } => format!("(local):{root}"),
            };
            match remote_workspace_mcp::check_workspace(&ws.endpoint).await {
